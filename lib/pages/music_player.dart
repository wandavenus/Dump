...
class _MusicPlayerState extends State<MusicPlayer> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (routes != null && routes.containsKey('index')) {
        setState(() { currentIndex = routes['index'] as int; });
  ...
  void setupAudioPlayer() {
    ...
    audioPlayer.processingStateStream.listen((state) {
      if (state == ProcessingState.completed) {
        setState(() { position = Duration.zero; isPlaying = false; });
        final routes = ModalRoute.of(context)?.settings.arguments as Map<String, dynamic>?;
        final List<LocalSong> allSongs = List<LocalSong>.from(routes?['songs'] ?? []);
  ...
  Future<void> _loadSong(int index) async {
    setState(() { isLoading = true; currentIndex = index; });
    AudioService.currentIndex = currentIndex;
      ...
      await audioPlayer.setAudioSource(
        AudioSource.file(
          ...
          tag: MediaItem(
            ...
            artUri: Uri.parse('content://media/external/audio/albumart/${selectedSong.albumId}'),
          ),
        ),
      );
  ...
  Widget build(BuildContext context) {
    final coverSize = MediaQuery.of(context).size.width - (22 * 2);
    return Scaffold(
      extendBodyBehindAppBar: true,
      body: GestureDetector(
        onVerticalDragUpdate: (details) {
          setState(() { dragOffset = (dragOffset + details.delta.dy).clamp(0.0, 500.0); });
        },
        onVerticalDragEnd: (details) {
          if (dragOffset > 200) { Navigator.pop(context); } else { setState(() { dragOffset = 0; }); }
        },
                ...
                children: [
                  ...
                  DecoratedBox(
                    decoration: BoxDecoration(
                      boxShadow: const [BoxShadow(color: Color.fromARGB(30, 0, 0, 0), blurRadius: 4, spreadRadius: 0)],
                      shape: BoxShape.rectangle,
                      borderRadius: BorderRadius.circular(3.0),
                    ),
                    child: ClipPath(
                      clipper: ShapeBorderClipper(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(3.0))),
                      child: SizedBox(
                        width: coverSize,
                        height: coverSize,
                        child: SongArtwork(albumId: AudioService.currentSong?.albumId ?? 0, size: coverSize),
                      ),
                    ),
                  ),
                  const SizedBox(height: 60),
                        ...
                        Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Container(
                                decoration: const BoxDecoration(shape: BoxShape.circle, color: Color.fromARGB(158, 136, 136, 136)),
                                child: const Icon(Icons.more_horiz_rounded, size: 29, color: Color.fromARGB(255, 255, 255, 255)),
                              ),
                            ],
                          ),
                        ),
                  ...
                  Container(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(formatTime(position), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                  ...
                  Row(
                    ...
                    children: [
                      IconButton(
                        icon: const Icon(Icons.fast_rewind_rounded, size: 70, color: Color.fromARGB(255, 255, 255, 255)),
                        onPressed: () {
                          if (currentIndex > 0) {
                      ...
                      IconButton(
                        icon: Icon(isPlaying ? Icons.pause_rounded : Icons.play_arrow_rounded, size: 70, color: const Color.fromARGB(255, 255, 255, 255)),
                        onPressed: () { if (isPlaying) { audioPlayer.pause(); } else { audioPlayer.play(); } },
                      ),
                      IconButton(
                        icon: const Icon(Icons.fast_forward_rounded, size: 70, color: Color.fromARGB(255, 255, 255, 255)),
                        onPressed: () {
                          final routes = ModalRoute.of(context)?.settings.arguments as Map<String, dynamic>?;
                          final List<LocalSong> allSongs = List<LocalSong>.from(routes?['songs'] ?? []);
                          if (currentIndex < allSongs.length - 1) {
                              ...
                              builder: (context) {
                                return Container(
                                  ...
                                  child: Column(
                                    children: [
                                      const SizedBox(height: 10),
                                      const Text('Lyrics', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                                      const SizedBox(height: 20),
                                      Expanded(
                                        child: SingleChildScrollView(
                                          child: Text(lyrics, textAlign: TextAlign.center, style: const TextStyle(fontSize: 20, height: 2)),
                                        ),
                                      ),
                                    ],
                                  ),
                                      ...
                                      Expanded(
                                        child: ListView.builder(
                                          ...
                                          itemBuilder: (context, index) {
                                            final song = AudioService.currentPlaylist[index];
                                            return ListTile(
                                              title: Text(song.title, maxLines: 1, overflow: TextOverflow.ellipsis),
                                              subtitle: Text(song.artist ?? 'Unknown Artist', maxLines: 1, overflow: TextOverflow.ellipsis),
                                              trailing: index == AudioService.currentIndex ? const Icon(Icons.equalizer) : null,
                                            );
                                          },
                                        ),
                                      ),
