...
class HttpTemplateExtensionRuntime implements ExtensionRuntime {
  HttpTemplateExtensionRuntime(this.manifest, {this._client});
  ...
  Future<List<OnlineTrack>> search(String query) async {
    final template = manifest.searchUrl;
    if (template == null || template.isEmpty) return const [];
    final client = _client ?? http.Client();
    try {
      final url = template.replaceAll(
        '{query}',
      ...
      return list
          .whereType<Map>()
          .map(
            (e) => OnlineTrack.fromJson(e.cast<String, dynamic>(), extensionId),
          )
  ...
  Future<String?> resolveDownloadUrl(OnlineTrack track) async {
    ...
    if (template == null || template.isEmpty) {
      return track.id.startsWith('http') ? track.id : null;
    }
