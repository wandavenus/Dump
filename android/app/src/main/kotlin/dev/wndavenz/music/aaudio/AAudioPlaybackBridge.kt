...
class AAudioPlaybackBridge(private val context: Context) : EventChannel.StreamHandler {
    ...
    fun handle(call: MethodCall, result: MethodChannel.Result) {
        try {
            when (call.method) {
                ...
                "seek" -> { pendingSeekUs = (call.argument<Int>("position") ?: 0) * 1000L; nativeFlush(); result.success(null) }
    ...
    private fun decode(path: String) {
        ...
        try {
            ...
            val mime = inputFormat.getString(MediaFormat.KEY_MIME) ?: throw IllegalStateException("No MIME")
            ...
            while (decoding.get() && !sawOutputEnd) {
                ...
                if (seekUs >= 0) {
                    ...
                    codec.flush(); pendingSeekUs = -1L; sawInputEnd = false; nativeFlush()
                }
                if (!sawInputEnd) {
                    ...
                    if (inIndex >= 0) {
                        ...
                        if (size < 0) { codec.queueInputBuffer(inIndex, 0, 0, 0, MediaCodec.BUFFER_FLAG_END_OF_STREAM); sawInputEnd = true }
                ...
                if (outIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    ...
                    emit(mapOf("type" to "format", "sampleRate" to rate, "channelCount" to channels, "mimeType" to "audio/raw"))
                    nativeStart(); emitState("playing")
                } else if (outIndex >= 0) {
                    ...
                    emit(mapOf("type" to "position", "position" to info.presentationTimeUs / 1000L))
            ...
            codec?.stop(); codec?.release(); extractor.release(); decoding.set(false)
        }
    }

    private fun stopDecode() { decoding.set(false); decoderThread?.join(500); decoderThread = null }
    private fun emitTrack() { emit(mapOf("type" to "track", "track" to queue.getOrNull(index)?.plus("index" to index))) }
    ...
    private fun abandonFocus() { if (Build.VERSION.SDK_INT >= 26) focusRequest?.let { audioManager.abandonAudioFocusRequest(it) } }
    ...
    companion object { init { System.loadLibrary("aaudio_engine") } }
}
